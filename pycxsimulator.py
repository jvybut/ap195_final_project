...
class GUI:
    ...
    def initGUI(self):
        ...
        self.status = Label(self.rootWindow, width=40, height=3,
                            relief=SUNKEN, bd=1, textvariable=self.statusText)
        ...
        self.buttonStep = Button(
            self.frameRun, width=30, height=2, text='Step Once', command=self.stepOnce)
        self.buttonStep.pack(side=TOP, padx=5, pady=5)
        self.showHelp(self.buttonStep, "Steps the simulation only once")

        # buttonReset
        self.buttonReset = Button(
            self.frameRun, width=30, height=2, text='Reset', command=self.resetModel)
        ...
        lab = Label(can, width=25, height=1, text="Step size ",
        ...
        self.showHelp(
            self.stepScale, "Skips model redraw during every [n] simulation steps\nResults in a faster model run.")
        self.stepScale.pack(side='left')

        can.pack(side='top')

        can = Canvas(self.frameSettings)
        lab = Label(can, width=25, height=1, text="Step visualization delay in ms ",
                    justify=LEFT, anchor=W, takefocus=0)
        lab.pack(side='left')
        ...
        self.showHelp(
            self.stepDelay, "The visualization of each step is delays by the given number of milliseconds.")
        ...
        for variableSetter in self.parameterSetters:
            ...
            lab = Label(can, width=25, height=1,
                        text=variableSetter.__name__+" ", anchor=W, takefocus=0)
            lab.pack(side='left')
    ...
    def stepModel(self):
        if self.running:
            ...
            self.rootWindow.after(
                int(self.timeInterval*1.0/self.stepSize), self.stepModel)
    ...
    def start(self, func=[]):
        if len(func) == 3:
            ...
            if (self.modelInitFunc.__doc__ != None and len(self.modelInitFunc.__doc__) > 0):
                ...
                self.textInformation.insert(
                    END, self.modelInitFunc.__doc__.strip())
